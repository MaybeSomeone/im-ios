...
class CSCB : public zlead::stn::ConnectionStatusCallback {
public:
  CSCB(id<ConnectionStatusDelegate> delegate) : m_delegate(delegate) {
  }
  void onConnectionStatusChanged(zlead::stn::ConnectionStatus connectionStatus) {
    if (m_delegate) {
...
class RPCB : public zlead::stn::ReceiveMessageCallback {
public:
    RPCB(id<ReceiveMessageDelegate> delegate) : m_delegate(delegate) {}
    
    void onReceiveMessage(const std::list<zlead::stn::TMessage> &messageList, bool hasMore) {
...
WFCCUserInfo* convertUserInfo(const zlead::stn::TUserInfo &tui) {
...
NSArray<WFCCUserInfo *>* converUserInfos(const std::list<zlead::stn::TUserInfo> &userInfoList) {
    NSMutableArray *out = [[NSMutableArray alloc] init];
    for (std::list<zlead::stn::TUserInfo>::const_iterator it = userInfoList.begin(); it != userInfoList.end(); it++) {
        [out addObject:convertUserInfo(*it)];
    }
    return out;
}

WFCCGroupInfo* convertGroupInfo(const zlead::stn::TGroupInfo &tgi) {
...
extern WFCCChannelInfo *convertProtoChannelInfo(const zlead::stn::TChannelInfo &tci);

NSArray<WFCCGroupInfo *>* convertGroupInfos(const std::list<zlead::stn::TGroupInfo> &groupInfoList) {
    NSMutableArray *out = [[NSMutableArray alloc] init];
    for (std::list<zlead::stn::TGroupInfo>::const_iterator it = groupInfoList.begin(); it != groupInfoList.end(); it++) {
        [out addObject:convertGroupInfo(*it)];
    }
    return out;
}
NSArray<WFCCChannelInfo *>* convertChannelInfos(const std::list<zlead::stn::TChannelInfo> &channelInfoList) {
    NSMutableArray *out = [[NSMutableArray alloc] init];
    for (std::list<zlead::stn::TChannelInfo>::const_iterator it = channelInfoList.begin(); it != channelInfoList.end(); it++) {
        [out addObject:convertProtoChannelInfo(*it)];
    }
    return out;
}

class GUCB : public zlead::stn::GetUserInfoCallback {
  public:
  GUCB(id<RefreshUserInfoDelegate> delegate) : m_delegate(delegate) {}
  
  void onSuccess(const std::list<zlead::stn::TUserInfo> &userInfoList) {
      if(m_delegate) {
...
class GGCB : public zlead::stn::GetGroupInfoCallback {
  public:
  GGCB(id<RefreshGroupInfoDelegate> delegate) : m_delegate(delegate) {}
  
  void onSuccess(const std::list<zlead::stn::TGroupInfo> &groupInfoList) {
...
class GGMCB : public zlead::stn::GetGroupMembersCallback {
public:
    GGMCB(id<RefreshGroupMemberDelegate> delegate) : m_delegate(delegate) {}
    
    void onSuccess(const std::string &groupId, const std::list<zlead::stn::TGroupMember> &groupMemberList) {
...
class GCHCB : public zlead::stn::GetChannelInfoCallback {
public:
    GCHCB(id<RefreshChannelInfoDelegate> delegate) : m_delegate(delegate) {}
    
    void onSuccess(const std::list<zlead::stn::TChannelInfo> &channelInfoList) {
...
class GFLCB : public zlead::stn::GetMyFriendsCallback {
public:
...
class GFRCB : public zlead::stn::GetFriendRequestCallback {
public:
...
class GSCB : public zlead::stn::GetSettingCallback {
public:
...
- (void)onConnectionStatusChanged:(ConnectionStatus)status {
  if (!_logined || kConnectionStatusRejected == status) {
    ...
    if((int)status == (int)zlead::stn::kConnectionStatusServerDown) {
...
- (void)suspend {
  if(_bgTaskId != UIBackgroundTaskInvalid) {
      ...
      if (zlead::stn::GetTaskCount() > 0 && self.backgroudRunTime < 60) {
          [self checkBackGroundTask];
      } else {
    zlead::stn::Reset();
...
- (void)onAppResume {
  ...
  zlead::stn::MakesureLonglinkConnected();
  [self endBgTask];
}
...
- (void) createMars {
  zlead::app::SetCallback(zlead::app::AppCallBack::Instance());
  zlead::stn::setConnectionStatusCallback(new CSCB(self));
  zlead::stn::setReceiveMessageCallback(new RPCB(self));
  zlead::stn::setRefreshUserInfoCallback(new GUCB(self));
  zlead::stn::setRefreshGroupInfoCallback(new GGCB(self));
    zlead::stn::setRefreshGroupMemberCallback(new GGMCB(self));
  zlead::stn::setRefreshChannelInfoCallback(new GCHCB(self));
  zlead::stn::setRefreshFriendListCallback(new GFLCB(self));
    zlead::stn::setRefreshFriendRequestCallback(new GFRCB(self));
  zlead::stn::setRefreshSettingCallback(new GSCB(self));
  zlead::baseevent::OnCreate();
}
- (void)connect:(NSString *)host port:(int)port {
    zlead::stn::Connect([host UTF8String], port);
...
- (long long)serverDeltaTime {
    return zlead::stn::getServerDeltaTime();
}
...
- (void)connect:(NSString *)userId token:(NSString *)token {
  _logined = YES;
    zlead::app::AppCallBack::Instance()->SetAccountUserName([userId UTF8String]);
    [self createMars];
    self.userId = userId;
    self.passwd = token;
    if(!zlead::stn::setAuthInfo([userId cStringUsingEncoding:NSUTF8StringEncoding], [token cStringUsingEncoding:NSUTF8StringEncoding])) {
        return;
    }
...
- (void)disconnect:(BOOL)clearSession {
  ...
  if (zlead::stn::getConnectionStatus() != zlead::stn::kConnectionStatusConnected && zlead::stn::getConnectionStatus() != zlead::stn::kConnectionStatusReceiving) {
    [self destroyMars];
  } else {
    zlead::stn::Disconnect(clearSession ? 8 : 0);
  }
}
...
// event reporting
- (void)reportEvent_OnForeground:(BOOL)isForeground {
    zlead::baseevent::OnForeground(isForeground);
}
...
- (void)setDeviceToken:(NSString *)token {
  ...
  if (!self.isLogined || self.currentConnectionStatus != kConnectionStatusConnected) {
    ...
    zlead::stn::setDeviceToken([appName UTF8String], [token UTF8String], zlead::app::AppCallBack::Instance()->GetPushType());
}
...
- (void)setVoipDeviceToken:(NSString *)token {
    ...
    NSString *appName =
    [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleIdentifier"];
    zlead::stn::setDeviceToken([appName UTF8String], [token UTF8String], 2);
}
...
-(void) ReachabilityChange:(UInt32)uiFlags {
    if ((uiFlags & kSCNetworkReachabilityFlagsConnectionRequired) == 0) {
        zlead::baseevent::OnNetworkChange();
    }
}
